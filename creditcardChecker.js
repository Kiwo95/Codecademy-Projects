...
function idInvalidCardCompanies(nestedArr){
  ...
  for (let i = 0; i < nestedArr.length; i++){
    if (nestedArr[i][0] > 2 && nestedArr [i][0] < 7){
      let digit = nestedArr[i][0];
      switch (digit){
        case 3:
...
// Find all invalid cards in the batch
const invalidCards = findInvalidCards(batch);
console.log(invalidCards);

// Find which companies have issued invalid cards
